        ...
        <div className="container-custom space-y-12">
          {caseStudies.map((study, i) => (
            <AnimatedSection
              ...
              delay={i * 0.1}
              id={study.id}
              className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center p-8 rounded-2xl bg-navy-800 border border-navy-700"
            >
              {/* Visual side */}
              <div
                className={`h-64 rounded-xl bg-gradient-to-br ${study.gradient} bg-navy-700 flex items-center justify-center ${i % 2 === 1 ? "lg:order-2" : ""}`}
              >
                <div className="text-center px-6">
                  <p className="text-6xl font-extrabold text-white/10 uppercase tracking-widest">
                    {study.client}
                  </p>
                  <p className="text-4xl font-extrabold text-teal mt-2">{study.result}</p>
                  <p className="text-slate-400 text-sm">{study.resultLabel}</p>
                </div>
              </div>
              ...
              <div className={i % 2 === 1 ? "lg:order-1" : ""}>
                ...
                <p className="text-slate-400 leading-relaxed mb-5">{study.description}</p>
                <div className="flex flex-wrap gap-2 mb-6">
                  {study.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-3 py-1 rounded-md bg-navy-700 text-slate-300 text-xs font-medium"
                    >
                      {tag}
                    </span>
                  ))}
                </div>
                <div className="flex items-center gap-3 p-4 rounded-xl bg-navy-700/50">
                  <span className="text-2xl font-extrabold text-teal">{study.result}</span>
                  <span className="text-slate-300 text-sm">{study.resultLabel}</span>
                </div>
              </div>
            </AnimatedSection>
          ))}
