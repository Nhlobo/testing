...
export default async function BlogPostPage({ params }: Params) {
  ...
  return (
    <div className="pt-24">
      <article className="section-padding bg-navy-950">
        <div className="container-custom max-w-3xl">
          ...
          <p className="text-slate-400 text-lg leading-relaxed mb-8 border-l-4 border-accent pl-4 italic">
            {post.excerpt}
          </p>

          {/* Content — rendered as pre-formatted text since content is markdown-like */}
          <div className="prose prose-invert prose-lg max-w-none">
            {post.content.split("\n").map((line, i) => {
              ...
              if (line.startsWith("- [ ] ")) {
                return (
                  <div key={i} className="flex items-center gap-2 text-slate-300 mb-2">
                    <span className="w-4 h-4 border border-slate-500 rounded shrink-0" />
                    {line.replace("- [ ] ", "")}
                  </div>
                );
              }
              if (line.startsWith("- ")) {
                return (
                  <li key={i} className="text-slate-300 mb-1 ml-4 list-disc">
                    {line.replace("- ", "")}
                  </li>
                );
              }
          ...
          {/* Author */}
          <div className="mt-12 pt-8 border-t border-navy-700 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-400 text-sm">Mapengo Innovations</p>
            </div>
          </div>

          {/* CTA */}
          <div className="mt-12 p-8 rounded-2xl bg-navy-800 border border-navy-700 text-center">
            <h3 className="text-white font-bold text-xl mb-3">Ready to Apply This to Your Business?</h3>
